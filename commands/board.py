...
class CmdBoard(MuxCommand):
    ...
    Admin Commands:
        |wboard/new <name>0|n - Create a new board
    ...
    def func(self):
        ...
        if not self.args and not self.switches:
            ...
            for board in boards:
                if board.access(caller, "read"):
                    ...
                    latest = "-"
                    if posts:
                        latest_post = posts[0][0]  # First post is newest, [0] is the post object
                        latest = latest_post.date_created.strftime("%Y-%m-%d %H:%M")
        ...
        if not self.switches:
            ...
            # Debug output
            caller.msg(f"DEBUG: Attempting to delete post {post.id}")
            result = board.delete_post(caller, post)
            caller.msg(f"DEBUG: Delete operation returned: {result}")
